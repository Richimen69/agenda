  ...
  try {
    ...
    if (!data.fullName || !data.source) {
      return res.status(400).json({
        success: false,
        error: "fullName y source son obligatorios",
      });
    }
    ...
    // Devolvemos con comments incluido para que el frontend no tenga que refetch
...
export const updateLead = async (req, res) => {
  try {
    ...
    const updatedLead = await prisma.$transaction(async (tx) => {
      ...
      // Auto-log solo si el estado realmente cambió
