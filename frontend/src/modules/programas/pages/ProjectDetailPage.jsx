...
export default function ProjectDetailPage({ creatorId }) {
  ...
  const allActions = actionTree.flatMap((a) => [a, ...(a.children || [])]);

  // Agrupamos el rendimiento por cada miembro del equipo (Soporte)
  const teamPerformance = team
    .map((member) => {
      // Buscamos todas las tareas asignadas a este empleado en específico
      const memberActions = allActions.filter(
        (a) => a.ownerId === member.user.id,
      );
      const memberKpis = memberActions.flatMap((a) => a.kpis || []);
    ...
    .filter((member) => member.kpis.length > 0);
  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6] p-6 text-content-main font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-6 border-b border-layout-border mb-6">
          <button
            onClick={() => setActiveTab("CHECKLIST")}
            className={`pb-3 text-sm font-bold transition-colors border-b-2 ${activeTab === "CHECKLIST" ? "border-brand text-brand" : "border-transparent text-content-muted hover:text-content-main"}`}
          >
            📋 Tareas y Bitácora
          </button>
          <button
            onClick={() => setActiveTab("ANALYTICS")}
            className={`pb-3 text-sm font-bold transition-colors border-b-2 ${activeTab === "ANALYTICS" ? "border-brand text-brand" : "border-transparent text-content-muted hover:text-content-main"}`}
          >
            📊 Tablero Analítico (BI)
          </button>
        </div>
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                {project.ratios &&
                  ...
                  teamPerformance.length > 0 && (
                    <div className="bg-white rounded-xl border border-layout-border shadow-sm overflow-hidden mt-6">
                      <div className="px-4 py-3 border-b border-layout-border flex items-center justify-between bg-slate-50">
                        <h3 className="font-bold text-slate-800 text-sm">
                        ...
                        <table className="w-full text-left border-collapse">
                          <thead>
                            <tr className="border-b border-slate-100 text-xs font-bold text-slate-500 uppercase tracking-wider">
                              ...
                              {/* Imprimimos las columnas basadas en la fórmula del Ratio que creó el Director */}
                              <th className="px-4 py-3 text-center">
                                {project.ratios[0].numeratorNames[0]}
                              </th>
                              <th className="px-4 py-3 text-center">
                                {project.ratios[0].denominatorNames[0]}
                              </th>
                              <th className="px-4 py-3 text-right text-brand">
                                {project.ratios[0].name}
                              </th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-50">
                            {teamPerformance.map((member) => {
                              // Usando el primer Ratio del proyecto como ejemplo para esta tabla
                              const ratio = project.ratios[0];

                              // Buscamos cuánto lleva ESTE ASESOR en el Numerador (Ej. Ventas)
                              const numValue = member.kpis
                                .filter((k) =>
                                  ratio.numeratorNames.includes(k.name),
                                )
                                .reduce((sum, k) => sum + k.currentValue, 0);

                              // Buscamos cuánto lleva ESTE ASESOR en el Denominador (Ej. Visitas)
                              const denValue = member.kpis
                                .filter((k) =>
                                  ratio.denominatorNames.includes(k.name),
                                )
                                .reduce((sum, k) => sum + k.currentValue, 0);

                              // Calculamos SU tasa de conversión personal
                              let result =
                                denValue > 0 ? numValue / denValue : 0;
                              if (ratio.unit.trim() === "%")
                                result = result * 100;
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3 text-right">
                                    <span
                                      className={`text-sm font-black ${result >= 15 ? "text-emerald-500" : "text-rose-500"}`}
                                    >
                                      {result.toFixed(2)} {ratio.unit}
                                    </span>
                                  </td>
                                </tr>
                              );
                ...
                <div className="bg-white rounded-xl border border-layout-border shadow-sm overflow-hidden flex flex-col h-125">
                  {/* Cabecera Bitácora */}
                  ...
                  <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-50/50 custom-scrollbar">
                    {project.projectComments?.length === 0 ? (
                      <div className="h-full flex items-center justify-center text-sm text-content-muted">
                        No hay comentarios aún. Escribe el primer mensaje para
                        el equipo.
                      </div>
                    ) : (
            ...
            <div className="flex items-center gap-2 mt-0.5">
              {isPurelyFinancial ? (
                ...
                >
                  ...
                  {financialKpi.target.toLocaleString("es-MX")}{" "}
                  {financialKpi.unit}
                </span>
              ) : (
