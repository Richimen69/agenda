  ...
  if (sessions.length === 0) {
    return (
      <div className="overflow-x-auto bg-white border border-gray-200 rounded-2xl shadow-sm">
        <p className="p-8 text-center text-gray-400 text-sm">El historial de transmisiones está vacío.</p>
      </div>
    );
  }
      ...
      <table className="w-full text-left border-collapse text-sm">
        <thead>
          <tr className="border-b border-gray-200 bg-gray-50 text-gray-400 text-xs uppercase font-bold">
            ...
            <th className="p-4">Fecha de Servicio</th>
        ...
        <tbody className="divide-y divide-gray-100">
          {sessions.map((session) => (
            <tr key={session.id} className="hover:bg-gray-50 text-gray-500">
              ...
              <td className="p-4">{new Date(session.createdAt).toLocaleDateString()}</td>
